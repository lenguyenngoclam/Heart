...
let Particle = (function() {  
  ...
  let ParticlePool = (function() {
    ...
    let particles,
        firstActive = 0, // Location in array that particle is present
        firstFree   = 0, // Location in the array that particle is free
    ...
    ParticlePool.prototype.add = function(x, y, dx, dy) {
      ...
      firstFree++; // Update free location to next location
  
      if (firstFree   == particles.length) firstFree   = 0;  // Reset location
      if (firstActive == firstFree       ) firstActive++;   // Update first active location 
    ...
    ParticlePool.prototype.update = function(deltaTime) {
        ...
        if (firstActive < firstFree) {
            // Update only particles in the range [firstActive, firstFree] (they are present only in this range)
            for (i = firstActive; i < firstFree; i++)
                particles[i].update(deltaTime);
        }
    
        if (firstFree < firstActive) {
            // Particles are present in range [0, firstFree - 1] and [firstActive, total number of particles in pool]
        ...
        while (particles[firstActive].age >= duration && firstActive != firstFree) {
            firstActive++; // Remove the particle here
            if (firstActive == particles.length) firstActive = 0; // Reset when reach end of pool
        }
...
let draw_heart = function (canvas, x_deg, y_deg, particle_size, particle_color, particle_velocity, particle_amount) {
  
    let context = canvas.getContext('2d'),
        particles = new ParticlePool(settings.particles.length), // initialize pool
    ...
    let image = (function () {
        ...
        function to(t) {
            ...
            point.x = settings.particles.size / 2 + point.x * settings.particles.size / 35;
            point.y = settings.particles.size / 2 - point.y * settings.particles.size / 35;
